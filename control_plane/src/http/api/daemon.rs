...
pub async fn delete_token(State(app): State<App>, id: String) -> Result<()> {
    let id: uuid::Uuid = id.parse()?;
    app.delete_daemon_token(id).await?;
    Ok(())
}

pub async fn join(State(app): State<App>, Json(join_request): Json<DaemonJoinRequest>) -> Result<impl IntoResponse> {

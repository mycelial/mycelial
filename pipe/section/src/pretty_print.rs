...
fn format_frame(lens: &[usize]) -> String {
    lens.iter().enumerate().map(|(pos, len)| {
        if pos == 0 {
...
fn format_row<T: AsRef<str>>(lens: &[usize], values: &[T]) -> String {
    lens.iter().zip(values.iter()).enumerate().map(|(pos, (len, name))| {
        if pos == 0 {
...
pub fn pretty_print(df: &dyn DataFrame) -> String {
    ...
    let values = columns.into_iter().map(|col| {
        col.map(|val| format!("{:?}", val)).collect::<Vec<String>>()
    }).collect::<Vec<_>>();
    let max_lens = values.iter().map(|val| val.iter().map(|s| s.len()).max().unwrap_or(0)).collect::<Vec<usize>>();
    let max_lens = max_lens.into_iter().zip(col_names.iter()).map(|(ln, col_name)| col_name.len().max(ln)).collect::<Vec<usize>>();
...
mod test {
    use super::*;
    use crate::message::{DataFrame, Column, ValueView};
    ...
    impl DataFrame for Values {
        fn columns(&self) -> Vec<Column<'_>> {
            vec![
                Column::new("ints", Box::new(self.ints.iter().map(|&x| ValueView::I64(x)))),
                Column::new("strings", Box::new(self.strings.iter().map(|s| ValueView::Str(s.as_str())))),
            ]
        }
    }
    ...
    fn test_format_header() {
        println!("{}", format_header(&[1, 2], &["a".to_string(), "b".to_string()]));
    }

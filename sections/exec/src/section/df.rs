//! ExecDf section
//! Section Executes user provided script for each row in incoming dataframe.
//!
...
impl<Input, Output, SectionChan> Section<Input, Output, SectionChan> for ExecDf
where
                    ...
                    msg = input.next().fuse() => {
                        ...
                        while let Some(chunk) = msg.next().await? {
                            match chunk {
                                ...
                                Chunk::Byte(_) => {
                                    Err("byte streams are not supported in 'DF' exec")?;
                                },
                            }
                        }
